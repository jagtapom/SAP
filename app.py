...
# Function to fetch OData Service Catalog
def fetch_service_catalog(hostname, port, username, password, ssl_verify):
    ...
    try:
        ...
        data = response.json()
        services = [
            f"{service['TechnicalName']} - {service['TechnicalServiceName']}"
...
selected_service = st.sidebar.selectbox("Select OData Service", services) if services else None
entity_sets = []
if selected_service:
